...
// Interface for MapComponent compatibility
interface MapSchool {
  id: number;
  name: string;
  city: string;
...
const Home = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (schoolsResult.data) {
        ...
        const schoolsData: School[] = schoolsResult.data.map(school => ({
          ...school,
          ratings: school.ratings as { overall: number; facility: number; faculty: number; activities: number } || {
            overall: 0,
            facility: 0,
            faculty: 0,
            activities: 0
  ...
  const stats = [
    { icon: Users, label: 'Schools Listed', value: `${schools.length}+`, color: 'text-primary' },
    { icon: MapPin, label: 'Cities Covered', value: `${cities.length}+`, color: 'text-success' },
          ...
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden" style={{ height: '600px' }}>
            <MapComponent 
              schools={filteredSchools.map(school => ({
                ...
                name: school.name,
                city: school.city,
                address: school.address || `${school.name}, ${school.city}`,
                coordinates: { lat: 20.2961, lng: 85.8245 }, // Default coordinates for Odisha
                ratings: school.ratings || { overall: 0, facility: 0, faculty: 0, activities: 0 }
              }))} 
